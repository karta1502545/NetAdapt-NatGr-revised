...
The task is to reduce numbers of parameters in a model, which can speed up the process per image, while the accuracy doesn't decrease too much.
...
The code can process now. Although the accuracy is not quite good(pretrained:69.14% -> 61.4% while 30% of latency reduced),
...
We can modify the minimum amount (>0) of remaining channels.(see in resnet18.py "minimum_channel_per_layer")
