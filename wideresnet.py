...
class WideResNet(nn.Module):
    ...
    def _forward_subnet(self, subnet_id, x):
        out = x  # in case n_blocks_per_subnet == 0F
        ...
        for layer_name in self.all_layers:
            if layer_name == "FC":
                ...
                if layer is not None:
                    ...
                    #print("=======================")
                    #print(self.get_cost(layer_name_table, layer.in_channels, layer.out_channels))
                    #print("=======================")
    ...
    def _get_pruning_score(self, layer_name):
        ...
        # declare variable
        weights = self.block2[0][0].conv1.weight.data
        weights_norm = torch.norm(weights.view(weights.size()[0], -1), p=2, dim=1)
        num_layers = 1
        if layer_name == 'block2_1_0':
            weights = self.block2[1][0].conv1.weight.data
            weights_norm = torch.norm(weights.view(weights.size()[0], -1), p=2, dim=1)
            num_layers = 1
        '''
    ...
    def load_bigger_state_dict(self, state_dict):
        ...
        print(f'Total : {len(state_dict)} Update: {len(pretrained_dict)}')
    ...
    def _remove_from_in_channels(self, layer_name, remaining_channels):
        ...
        :param remaining_channels: the offsets of the remaining channels"""
        # get attribute
        layer = self.block2[0][0].conv2
        if layer_name == 'block2_0_1':
            layer = self.block2[0][0].conv2
        elif layer_name == 'block2_1_1':
            layer = self.block2[0][1].conv2
        ...
        # set attribute
        if layer_name == 'block2_0_1':
            self.block2[0][0].conv2 = new_layer
        elif layer_name == 'block2_1_1':
            self.block2[0][1].conv2 = new_layer
    ...
    def _remove_from_out_channels(self, layer_name, remaining_channels): 
        ...
        :param layer_name: the name of the layer that will be pruned
        :param remaining_channels: the offsets of the remaining channels"""
        layer = self.block2[0][0].conv1 # conv_2_1_1
        if layer_name == 'block2_0_0':
            layer = self.block2[0][0].conv1
        elif layer_name == 'block2_1_0':
            layer = self.block2[0][1].conv1
        #else: 
            # ...
        num_remaining_channels = remaining_channels.size(0)
        ...
        # set attribute
        if layer_name == 'block2_0_0':
            self.block2[0][0].conv1 = new_layer
        elif layer_name == 'block2_1_0':
            self.block2[0][1].conv1 = new_layer
        #else:
            # ...

        # batchnorm layer # If I prune some channels of block2[0][0].conv1, block2[0][0].bn2 should be modified at the same time.
        bn_layer = self.block2[0][0].bn2
        if layer_name[:4] != "Skip":
            # get attribute of batchnorm layer 2 (bn2)
            if layer_name == 'block2_0_0':
                bn_layer = self.block2[0][0].bn2
            elif layer_name == 'block2_1_0':
                bn_layer = self.block2[0][1].bn2
            #else: 
                # ...
            ...
            # set attribute of batchnorm layer 2 (bn2)
            if layer_name == 'block2_0_0':
                self.block2[0][0].bn2 = new_bn_layer
            elif layer_name == 'block2_1_0':
                self.block2[0][1].bn2 = new_bn_layer
            #else: 
                # ...
